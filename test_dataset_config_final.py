...
sys.modules['advsecurenet.shared.types.configs.preprocess_config'] = MockModule(PreprocessConfig=MockPreprocessConfig)
...
def test_comprehensive_coverage():
    ...
    # Test full initialization
    user2 = dc.UserSplitConfig('user_id', 'validation', MockPreprocessConfig(), 
                              {'key': 'value'}, {'arg': 'value'}, '/path/to/data')
    ...
    create2 = dc.CreateDatasetCliConfig('create2', 5, MockPreprocessConfig(), 'custom_id',
                                       {'dataset': 'kwargs'}, {'constructor': 'args'},
                                       split_config, ['train', 'test'], 1000)
    ...
    # Test full initialization
    resolved2 = dc.ResolvedSplitConfig('resolved2', 8, 'train', MockPreprocessConfig(),
                                      {'key': 'value'}, {'arg': 'value'}, '/resolved/path')
    ...
    split_config = {'custom_train': dc.UserSplitConfig(), 'custom_test': dc.UserSplitConfig()}
    config1 = dc.CreateDatasetCliConfig('test', split_config=split_config, load_splits=['ignored'])
    user_splits1 = dc._get_user_splits(config1)
    assert user_splits1 == split_config
    
    # Test with load_splits only
    config2 = dc.CreateDatasetCliConfig('test', load_splits=['train', 'validation', 'test'])
    user_splits2 = dc._get_user_splits(config2)
    expected_splits = {name: dc.UserSplitConfig() for name in ['train', 'validation', 'test']}
    ...
    config4 = dc.CreateDatasetCliConfig('test', split_config={}, load_splits=['custom_split'])
    ...
    global_config = dc.CreateDatasetCliConfig('global_test', 15, MockPreprocessConfig(),
    ...
    user_config = dc.UserSplitConfig('user_identifier', 'validation', MockPreprocessConfig(),
                                    {'user': 'kwargs'}, {'user': 'args'}, '/user/path')
    ...
    resolved_split3 = dc._create_resolved_split(global_config, 'validation', user_partial)
    assert resolved_split3.identifier == 'partial_id'
    assert resolved_split3.num_classes == 15  # Global value
    assert resolved_split3.source_split_name == 'validation'  # Global overridden by split_name
    ...
    global_none = dc.CreateDatasetCliConfig('none_test', dataset_kwargs=None, constructor_args=None)
    ...
    # Test simple config
    simple_config = dc.CreateDatasetCliConfig('simple_dataset', load_splits=['train', 'test'])
    ...
    complex_config = dc.CreateDatasetCliConfig('complex_dataset', split_config=split_config,
    ...
    try:
        empty_config = dc.CreateDatasetCliConfig('empty_dataset', split_config=None, load_splits=[])
