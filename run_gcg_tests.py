...
TEST_MODULES = {
    ...
    "conversation": ["test_template_adapter.py", "test_template_utils.py"],
    "models": ["test_model_worker.py", "test_embedding_utils.py"],
    "prompts": ["test_attack_prompt.py", "test_prompt_manager.py"],
    "gcg": ["test_gcg_attack.py"],
}
...
def run_tests(module=None, coverage=False, verbose=False, html_report=False):
    ...
    if coverage:
        cmd.extend(["--cov=" + str(GCG_SRC_DIR), "--cov-report=term-missing"])
...
Examples:
  ...
  python run_gcg_tests.py --html --coverage
    ...
    parser.add_argument(
        "--summary", action="store_true", help="Show test summary and exit"
    )
