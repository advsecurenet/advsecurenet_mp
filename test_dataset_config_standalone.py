...
dataset_config_path = os.path.join(
    root_dir, "advsecurenet", "shared", "types", "configs", "dataset_config.py"
)
...
preprocess_config_path = os.path.join(
    root_dir, "advsecurenet", "shared", "types", "configs", "preprocess_config.py"
)
...
def test_all_functions():
    ...
    base_config2 = BaseDatasetCliConfig(
        dataset_name="test2", num_classes=5, preprocessing=PreprocessConfig()
    )
    ...
    attacks_config2 = AttacksDatasetCliConfig(
        dataset_name="attack_test2", random_sample_size=100
    )
    ...
    resolved_config = ResolvedDatasetConfig(dataset_name="resolved_test", splits=splits)
    ...
    resolved_config2 = ResolvedDatasetConfig(
        dataset_name="resolved_test2", splits=splits, random_sample_size=1000
    )
    ...
    config4 = CreateDatasetCliConfig(
        dataset_name="test", load_splits=["train", "test", "val"]
    )
    ...
    config6 = CreateDatasetCliConfig(
        dataset_name="test", split_config={}, load_splits=["custom"]
    )
    result6 = _get_user_splits(config6)
    assert result6 == ["custom"]

    # Test empty load_splits falls back to default
    config7 = CreateDatasetCliConfig(dataset_name="test", load_splits=[])
    ...
    global_none = CreateDatasetCliConfig(
        dataset_name="none_test", dataset_kwargs=None, constructor_args=None
    )
    ...
    simple_config = CreateDatasetCliConfig(
        dataset_name="simple", load_splits=["train", "test"]
    )
    ...
    try:
        empty_config = CreateDatasetCliConfig(dataset_name="empty", load_splits=[])
