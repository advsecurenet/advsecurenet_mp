...
def test_save_checkpoint(mock_save, train_config):
    from advsecurenet.trainer import trainer_logic
    trainer = Trainer(train_config)
    trainer_logic.save_checkpoint(epoch=1, optimizer=trainer.optimizer, model=trainer.model, checkpoint_path="test_path")
...
def test_should_save_checkpoint(train_config):
    from advsecurenet.trainer import trainer_logic
    trainer = Trainer(train_config)
    assert trainer_logic.should_save_checkpoint(epoch=1, save_checkpoint=True, checkpoint_interval=1) is True
...
def test_train_method_runs():
    ...
    model = ModelFactory.create_model(model_name="CustomCifar10Model", pretrained=False, architecture={"num_classes": 10}) 
...
def test_trainer_with_checkpoint_loading(mock_load_checkpoint, train_config, device):
    ...
    with patch("advsecurenet.trainer.trainer_logic.assign_device_to_optimizer_state") as mock_assign_device:
        trainer = Trainer(train_config)
...
def test_trainer_with_differential_privacy_invalid_model(mock_setup_privacy, mock_fix, mock_is_valid, train_config):
    ...
    dp_config = DifferentialPrivacyConfig(enable=True, noise_multiplier=1.0, delta=1e-5, max_grad_norm=1.0)
...
@patch("opacus.validators.ModuleValidator.is_valid")
@patch("advsecurenet.trainer.trainer.setup_privacy_engine")
def test_trainer_with_differential_privacy_valid_model_with_inplace_false(mock_setup_privacy, mock_is_valid, train_config):
...
@patch("opacus.validators.ModuleValidator.is_valid")
@patch("advsecurenet.trainer.trainer.setup_privacy_engine")
def test_trainer_with_differential_privacy_valid_model_without_inplace_false(mock_setup_privacy, mock_is_valid, train_config):
...
def test_trainer_differential_privacy_disabled(train_config):
    ...
    dp_config = DifferentialPrivacyConfig(enable=False, noise_multiplier=1.0, delta=1e-5, max_grad_norm=1.0)
    train_config.differential_privacy_config = dp_config
...
def test_trainer_train_with_global_patch(mock_execute_loop, mock_non_inplace, train_config):
