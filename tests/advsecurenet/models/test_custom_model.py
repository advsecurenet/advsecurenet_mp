...
class MockCustomModel:
    def __init__(self, num_classes, num_input_channels, **kwargs):
        self.architecture = {"num_classes": num_classes, "num_input_channels": num_input_channels}
...
def test_load_model(mock_import_module, custom_model_config):
    ...
    assert model.model.architecture["num_classes"] == custom_model_config.architecture["num_classes"]
    assert model.model.architecture["num_input_channels"] == custom_model_config.architecture["num_input_channels"]
...
def test_load_model_class_not_found(custom_model_config):
    with patch('importlib.import_module') as mock_import_module:
        ...
        with pytest.raises(ValueError, match="Model class MockCustomModel not found in module advsecurenet.models.CustomModels.MockCustomModel"):
            model = CustomModel(custom_model_config)
...
def test_models(mock_abspath, mock_isfile, mock_listdir):
    fake_custom_model_path = 'your/path/to/advsecurenet/advsecurenet/models/CustomModels'
    custom_model_python_file_dir = 'your/path/to/advsecurenet/advsecurenet/models/custom_model.py'

    mock_abspath.return_value = custom_model_python_file_dir 
    mock_listdir.return_value = ['MockCustomModel.py', '__init__.py', 'some_other_file.txt']
    ...
    mock_isfile.assert_any_call(os.path.join(fake_custom_model_path, 'MockCustomModel.py'))
    mock_isfile.assert_any_call(os.path.join(fake_custom_model_path, '__init__.py'))
    mock_isfile.assert_any_call(os.path.join(fake_custom_model_path, 'some_other_file.txt'))
