...
def assert_transformers_getattr_called_with(
    mock_getattr, expected_class_name, expected_default=None
):
    ...
    for call in getattr_calls:
        if len(call[0]) >= 2 and call[0][1] == expected_class_name:
            ...
            if (
                hasattr(call_module, "__name__")
                and "transformers" in call_module.__name__
            ):
    ...
    ), f"Expected getattr call with transformers module and '{expected_class_name}' but got calls: {getattr_calls}"
...
@pytest.mark.essential
@pytest.mark.skip(
    reason="Complex mocking test with module identity issues between local and CI environments"
)
def test_load_model_pretrained_success_inferred(
    ...
    assert_transformers_getattr_called_with(
        mock_transformers_getattr, "MockHFModel", None
    )
...
def test_load_model_pretrained_success_manual_override(
    ...
    # Use helper function to avoid module identity issues
    assert_transformers_getattr_called_with(
        mock_transformers_getattr, "SomeOtherModel", None
    )
    mock_issubclass.assert_called_with(MockHFModel, torch.nn.Module)
...
def test_load_model_fallback_to_automodel_inferred_load_error(
    ...
    assert (
        len(bert_calls) >= 1
    ), f"Expected getattr call with transformers module and 'BertForMaskedLM' but got calls: {getattr_calls}"
