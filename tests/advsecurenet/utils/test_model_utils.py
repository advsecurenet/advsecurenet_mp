...
from advsecurenet.utils.model_utils import download_weights, load_model, save_model, non_inplace_operations
...
def test_save_model_default_filepath():
    ...
    with patch("advsecurenet.utils.model_utils.pkg_resources.resource_filename") as mock_resource:
        with tempfile.TemporaryDirectory() as temp_dir:
...
def test_download_weights_with_filename(mock_tqdm, mock_get):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        expected_url = f"https://advsecurenet.s3.eu-central-1.amazonaws.com/weights/{filename}"
...
def test_download_weights_missing_parameters():
    ...
    with pytest.raises(ValueError, match="Both model_name and dataset_name must be provided if filename is not specified."):
...
def test_download_weights_http_error(mock_remove, mock_get):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with pytest.raises(Exception, match="HTTP 404 Not Found"):
            download_weights(model_name=model_name, dataset_name=dataset_name, save_path=temp_dir)
...
def test_download_weights_keyboard_interrupt(mock_remove, mock_get):
    ...
    mock_response.headers = {"content-length": "4"}
    mock_response.raise_for_status = MagicMock()
    mock_response.iter_content = MagicMock(side_effect=KeyboardInterrupt("User interrupted"))
    mock_get.return_value = mock_response
...
def test_download_weights_creates_directory(mock_tqdm, mock_get):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        download_weights(model_name=model_name, dataset_name=dataset_name, save_path=nonexistent_path)
