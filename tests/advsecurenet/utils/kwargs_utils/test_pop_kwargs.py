...
class TestPopStringKeysFromDict:
    ...
    def test_pop_nested_keys_dot_notation(self):
        ...
        expected = {
            "config": {"model": {"type": "classifier"}, "training": {"epochs": 10}}
        }
        assert result == expected
    ...
    def test_pop_deeply_nested_keys(self):
        ...
        original = {
            "deep": {
                "nested": {
                    "structure": {
                        "with": {
                            "many": {"levels": "target_value", "keep": "this_value"}
                        }
                    }
                }
            },
        ...
        result = pop_string_keys_from_dict(
            original, ["deep.nested.structure.with.many.levels"]
        )

        expected = {
            "deep": {
                "nested": {"structure": {"with": {"many": {"keep": "this_value"}}}}
            },
            "top": "level",
        }
        assert result == expected
    ...
    def test_pop_nested_keys_nonexistent_path(self):
        ...
        original = {"config": {"model": {"name": "bert"}}}
        result = pop_string_keys_from_dict(
            original, ["config.nonexistent.key", "missing.path"]
        )

        expected = {"config": {"model": {"name": "bert"}}}
        assert result == expected
    ...
    def test_pop_mixed_keys(self):
        ...
        result = pop_string_keys_from_dict(
            original, ["top_key", "config.model.name", "config.data.path"]
        )

        expected = {
            "config": {"model": {"version": "1.0"}, "data": {}},
            "another_top": "value2",
        }
        assert result == expected
    ...
    def test_pop_entire_nested_structure(self):
        ...
        original = {"keep": "this", "remove": {"entire": {"structure": "here"}}}
    ...
    def test_deep_copy_independence(self):
        ...
        original = {"nested": {"list": [1, 2, 3], "dict": {"inner": "value"}}}
        ...
        expected = {"nested": {"list": [1, 2, 3], "dict": {}}}
        assert result == expected
...
class TestRemoveNestedKeyRecursive:
    ...
    def test_remove_deeply_nested_key(self):
        ...
        dictionary = {
            "level1": {
                "level2": {
                    "level3": {"level4": {"target": "remove_this", "keep": "this"}}
                }
            }
        }

        _remove_nested_key_recursive(
            dictionary, ["level1", "level2", "level3", "level4", "target"]
        )

        expected = {"level1": {"level2": {"level3": {"level4": {"keep": "this"}}}}}
        assert dictionary == expected
    ...
    def test_remove_key_intermediate_non_dict(self):
        ...
        dictionary = {
            "config": {"model": "string_not_dict", "other": {"nested": "value"}}
        }
        original_dict = dictionary.copy()
