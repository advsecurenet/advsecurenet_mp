...
from advsecurenet.llm.GCG.experiments.cli_gcg import (
    gcg, attack, list_configs, test, quick, benchmark, evaluate_hf
)
...
class TestAttackCommand:
    ...
    def test_attack_basic(self, mock_cwd, mock_subprocess):
        ...
        result = self.runner.invoke(attack, [
    ...
    def test_attack_with_all_options(self, mock_cwd, mock_subprocess):
        ...
        result = self.runner.invoke(attack, [
            ...
            '--data-offset', '5',
            '--verbose',
            '--config-override', 'param1=value1',
            '--config-override', 'param2=value2'
        ])
    ...
    def test_attack_invalid_config_override(self):
        ...
        try:
            result = self.runner.invoke(attack, [
                '--config', temp_config,
                '--config-override', 'invalid_format'
            ])
...
class TestTestCommand:
    ...
    def test_test_command_success(self, mock_cwd):
        ...
        def mock_path_div(path_str):
            mock_path = MagicMock()
            if path_str in ["main.py", "configs/universal_config.py", "data/advbench/harmful_behaviors.csv"]:
    ...
    def test_test_command_missing_dependencies(self, mock_cwd):
        ...
        with patch.dict('sys.modules', {'torch': None, 'transformers': None}, clear=False):
