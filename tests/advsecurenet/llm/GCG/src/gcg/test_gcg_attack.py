...
def mock_model():
    ...
    model.return_value.logits = torch.tensor(
        [[[0.1, 0.2, 0.7], [0.3, 0.4, 0.3], [0.2, 0.6, 0.2]]]
    )
    return model
...
class TestTokenGradients:
    ...
    def test_token_gradients_basic(
        self,
        mock_get_embeddings,
        mock_get_embedding_matrix,
        mock_model,
        mock_embedding_weights,
        basic_input_ids,
    ):
        ...
        with patch("torch.nn.CrossEntropyLoss") as mock_loss_fn:
            ...
            with patch("torch.zeros") as mock_zeros:
                ...
                mock_one_hot.grad.clone.return_value = torch.tensor(
                ...
                mock_one_hot.__matmul__ = MagicMock(
                    return_value=mock_input_embeds.squeeze(0)
                )

                result = token_gradients(
                    mock_model, basic_input_ids, input_slice, target_slice, loss_slice
                )
    ...
    @patch("advsecurenet.llm.GCG.src.gcg.gcg_attack.get_embedding_matrix")
    @patch("advsecurenet.llm.GCG.src.gcg.gcg_attack.get_embeddings")
    def test_token_gradients_one_hot_creation(
        self,
        mock_get_embeddings,
        mock_get_embedding_matrix,
        mock_model,
        ...
        with patch("torch.zeros") as mock_zeros, patch("torch.nn.CrossEntropyLoss"):
    ...
    @patch("advsecurenet.llm.GCG.src.gcg.gcg_attack.get_embedding_matrix")
    @patch("advsecurenet.llm.GCG.src.gcg.gcg_attack.get_embeddings")
    def test_token_gradients_embedding_concatenation(
        self,
        mock_get_embeddings,
        mock_get_embedding_matrix,
        mock_model,
        ...
        with patch("torch.cat") as mock_cat, patch("torch.nn.CrossEntropyLoss"), patch(
            "torch.zeros"
        ) as mock_zeros:
    ...
    def test_token_gradients_device_handling(self):
        ...
        with patch(
            "advsecurenet.llm.GCG.src.gcg.gcg_attack.get_embedding_matrix"
        ) as mock_get_matrix, patch(
            "advsecurenet.llm.GCG.src.gcg.gcg_attack.get_embeddings"
        ) as mock_get_embeds, patch(
            "torch.zeros"
        ) as mock_zeros:
...
class TestGCGAttackPrompt:
    ...
    def test_gcg_attack_prompt_input_ids_device_transfer(self):
        ...
        with patch(
            "advsecurenet.llm.GCG.src.gcg.gcg_attack.token_gradients"
        ) as mock_token_gradients:
...
class TestGCGPromptManager:
    ...
    def test_gcg_prompt_manager_init(self, mock_super_init):
        ...
        manager = GCGPromptManager(
            ["goal"], ["target"], MagicMock(), MagicMock(), managers={"AP": MagicMock()}
        )
    ...
    def test_sample_control_basic(self):
        ...
        with patch.object(
            type(manager),
            "control_toks",
            new_callable=lambda: property(lambda self: torch.tensor([1, 2, 3, 4])),
        ):
            grad = torch.tensor(
                [
            ...
            with patch("torch.randint") as mock_randint:
                ...
                result = manager.sample_control(
                    grad, batch_size, topk, allow_non_ascii=True
                )
    ...
    def test_sample_control_non_ascii_filtering(self):
        ...
        with patch.object(
            type(manager),
            "control_toks",
            new_callable=lambda: property(lambda self: torch.tensor([1, 2, 3])),
        ):
            grad = torch.tensor(
                [
                    [0.5, 0.2, 0.8, 0.1, 0.3],
            ...
            with patch("torch.randint") as mock_randint:
                ...
                result = manager.sample_control(
                    grad, batch_size, topk, allow_non_ascii=False
                )
    ...
    def test_sample_control_topk_selection(self):
        ...
        with patch.object(
            type(manager),
            "control_toks",
            new_callable=lambda: property(lambda self: torch.tensor([10, 20])),
        ):
            grad = torch.tensor(
                [
            ...
            with patch("torch.randint") as mock_randint, patch(
                "torch.arange"
            ) as mock_arange:
    ...
    def test_sample_control_temperature_ignored(self):
        ...
        with patch.object(
            type(manager),
            "control_toks",
            new_callable=lambda: property(lambda self: torch.tensor([1, 2])),
        ):
            grad = torch.tensor(
                [[0.1, 0.9], [0.8, 0.2]]
            )  # 2 control positions, 2 vocab tokens
...
class TestGCGEdgeCases:
        ...
        with patch(
            ...
            try:
                with patch("torch.zeros") as mock_zeros:
                    mock_one_hot = MagicMock()
                    mock_one_hot.grad = MagicMock()
                    mock_one_hot.grad.clone.return_value = torch.tensor([]).reshape(
                        0, 100
                    )
                    mock_zeros.return_value = mock_one_hot

                    result = token_gradients(
                        model, input_ids, empty_slice, slice(3, 4), slice(2, 3)
                    )
    ...
    def test_gcg_prompt_manager_empty_control_toks(self):
        ...
        with patch.object(
            type(manager),
            "control_toks",
            new_callable=lambda: property(lambda self: torch.tensor([])),
        ):
    ...
    def test_step_control_weight_nonzero_comprehensive(
        self, mock_print, mock_tqdm, mock_gc_collect
    ):
        ...
        mock_prompt = MagicMock()
        mock_prompt.sample_control.return_value = torch.tensor(
            [[1, 2, 3], [4, 5, 6]]
        )  # batch_size=2
        mock_prompt.target_loss.return_value = torch.tensor([0.5, 0.6])
        mock_prompt.control_loss.return_value = torch.tensor(
            [0.1, 0.2]
        )  # This tests control_weight != 0 path
        ...
        with patch.object(attack, "get_filtered_cands") as mock_get_filtered:
            mock_get_filtered.return_value = [
                ["filtered_control1", "filtered_control2"]
            ]
    ...
    def test_step_verbose_progress_description_format(self):
        ...
        with patch.object(attack, "get_filtered_cands") as mock_filtered, patch(
            "advsecurenet.llm.GCG.src.gcg.gcg_attack.tqdm"
        ) as mock_tqdm:
    ...
    def test_step_final_calculations_and_cleanup(self):
        ...
        with patch.object(attack, "get_filtered_cands") as mock_filtered, patch(
            "advsecurenet.llm.GCG.src.gcg.gcg_attack.gc.collect"
        ) as mock_gc, patch("builtins.print") as mock_print:
    ...
    def test_step_realistic_integration(self):
        ...
        def mock_get_filtered_cands(
            model_idx, control_cand, filter_cand=True, curr_control=None
        ):
        ...
        def mock_worker_call(prompt, method, model, cand=None, return_ids=False):
            ...
            if method == "grad":
                ...
                mock_worker.results.get = lambda: (
                    torch.randn(1, 5, 10),
        ...
        mock_prompt.control_loss.return_value = torch.tensor(
            [0.1]
        )  # For control_weight test

        with patch("advsecurenet.llm.GCG.src.gcg.gcg_attack.tqdm") as mock_tqdm, patch(
            "builtins.print"
        ):
    ...
    def test_gcg_multi_prompt_attack_step_with_progress(self):
        ...
        with patch("torch.stack") as mock_stack, patch("tqdm.tqdm") as mock_tqdm:
