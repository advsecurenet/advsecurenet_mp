...
class TestProgressiveMultiPromptAttackInit:
    ...
    def test_multiple_workers(self, mock_worker, mock_managers):
        ...
        attack = ProgressiveMultiPromptAttack(
            goals=["goal"], targets=["target"], workers=workers, managers=mock_managers
        )
...
class TestProgressiveAttackRun:
    def test_run_basic_progressive_goals(self, basic_config):
        ...
        for i, mpa in enumerate(mpa_instances):
            ...
            mpa.run = MagicMock(
                return_value=(f"control_{i}", 0.5, 5)
            )  # Return 3 values
    ...
    def test_run_both_progressive(self, mock_worker, mock_managers):
        ...
        assert (
            mock_managers["MPA"].call_count == 3
        )  # 1goal+1worker, 2goals+1worker, 2goals+2workers
    ...
    def test_run_with_all_parameters(self, basic_config):
        ...
        mpa_instance.run = MagicMock(
            return_value=("final_control", 0.1, 20)
        )  # Return 20 steps to complete in one iteration
        basic_config["managers"]["MPA"].return_value = mpa_instance
    ...
    def test_run_empty_goals(self, mock_worker, mock_managers):
        attack = ProgressiveMultiPromptAttack(
            goals=[], targets=[], workers=[mock_worker], managers=mock_managers
        )
        ...
        mpa_instance.run = MagicMock(
            return_value=("empty_control", 0.0, 10)
        )  # Return 10 steps to complete
        mock_managers["MPA"].return_value = mpa_instance
        ...
        assert (
            mock_managers["MPA"].call_count == 1
        )  # Still called once with empty lists
...
class TestProgressiveAttackEdgeCases:
    ...
    def test_no_workers(self, mock_managers):
        attack = ProgressiveMultiPromptAttack(
            goals=["goal"], targets=["target"], workers=[], managers=mock_managers
        )
        ...
        mpa_instance.run = MagicMock(
            return_value=("no_worker_control", 0.5, 10)
        )  # Return 10 steps to complete
        mock_managers["MPA"].return_value = mpa_instance
        ...
        assert (
            mock_managers["MPA"].call_count == 1
        )  # Still called once with empty worker list
    ...
    def test_filter_mpa_kwargs(self, basic_config):
        ...
        expected = {"param1": "value1", "param2": "value2", "another": "value4"}
        assert result == expected

    def test_missing_managers(self, mock_worker):
        attack = ProgressiveMultiPromptAttack(
            goals=["goal"], targets=["target"], workers=[mock_worker], managers=None
        )
...
class TestProgressiveAttackIntegration:
    def test_complete_progressive_workflow(self, tmp_path):
        ...
        final_control = attack.run(
            n_steps=10, batch_size=16, test_steps=5, verbose=True
        )
        ...
        assert (
            final_control[0] == "control_2"
        )  # Last control (since we created 3 instances: 0, 1, 2)
        assert attack.control == "control_2"
