...
def mock_managers(mock_attack):
    ...
    return {"MPA": MagicMock(return_value=mock_attack)}
...
class TestEvaluateAttack:
    ...
    def _create_mock_prompt(
        self, eval_str="test prompt", test_new_toks=50, target="target1"
    ):
    ...
    def test_init_minimal_params(self, mock_worker):
        ...
        assert evaluator.test_prefixes == [
            "I'm sorry",
            "I apologize",
            "As an",
            "I'm just",
    ...
    def test_init_multiple_workers_assertion_error(self, mock_worker):
        ...
        workers = [
            mock_worker,
            mock_worker,
        ]  # Multiple workers should cause assertion error
    ...
    def test_filter_mpa_kwargs_with_mpa_args(self):
        ...
        result = EvaluateAttack.filter_mpa_kwargs(
            mpa_param1="value1", mpa_param2="value2", regular_param="value3"
        )
    ...
    def test_run_minimal(self, mock_empty_cache, mock_worker, mock_managers):
        ...
        result = evaluator.run(
            steps=1, controls=controls, batch_size=3, verbose=False
        )  # larger batch size
    ...
    def test_run_with_test_goals_and_targets(self, mock_worker, mock_managers):
        ...
        mock_worker.tokenizer.batch_decode.return_value = [
            "test_target1 output",
            "output2",
            "output3",
        ]
    ...
    def test_run_jailbreak_detection(self, mock_worker, mock_managers):
        ...
        mock_worker.model.generate.return_value = torch.tensor(
        ...
        mock_worker.tokenizer.decode.side_effect = [
            "input1",
            "input2",
            "input3",
            "input4",
    ...
    def test_run_exact_match_detection(self, mock_worker, mock_managers):
        ...
        evaluator = EvaluateAttack(
            goals=goals, targets=targets, workers=workers, managers=mock_managers
        )
    ...
    def test_run_jailbreak_and_exact_match_detection(self, mock_worker, mock_managers):
        ...
        mock_worker.tokenizer.batch_decode.return_value = [
            "I'm sorry, but target1",
            "Good answer without target",
            "Normal answer",
            "Another response",
        ]
        mock_worker.tokenizer.decode.side_effect = lambda x, **kwargs: "input"
    ...
    def test_run_max_new_len_parameter(self, mock_worker, mock_managers):
        ...
        controls = ["control1"]
        evaluator.run(
            steps=1, controls=controls, batch_size=5, max_new_len=100, verbose=False
        )
