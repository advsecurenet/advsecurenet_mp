...
class TestApplyOverrides:
    ...
    def test_apply_overrides_data_config(self):
        ...
        cfg = Config(
            ...
            data=DataConfig(train_file="original.jsonl", eval_file="original_eval.jsonl"),
            peft=PEFTConfig(enabled=False)
        )
        
        overrides = {
            "train_file": "new_train.jsonl",
            "eval_file": "new_eval.jsonl"
        }
    ...
    def test_apply_overrides_eval_file_none(self):
        ...
        assert result.data.eval_file == "existing_eval.jsonl"  # Unchanged because None is filtered
        assert isinstance(result.data, DataConfig)
...
class TestTrainCommand:
    ...
    @patch('advsecurenet.llm_finetuning.cli.run_training')
    @patch('advsecurenet.llm_finetuning.cli.load_yaml')
    def test_train_with_config_file_and_overrides(self, mock_load_yaml, mock_run_training):
    ...
    def test_train_flags_only_mode_missing_model_name(self):
        ...
        # Verify
        assert result.exit_code != 0
        assert "When --config is not provided, you must pass: --model-name" in result.output
    ...
    def test_train_flags_only_mode_missing_train_file(self):
        ...
        # Verify
        assert result.exit_code != 0
        assert "When --config is not provided, you must pass: --train-file" in result.output
    ...
    def test_train_flags_only_mode_missing_both_required(self):
        ...
        # Verify
        assert result.exit_code != 0
        assert "When --config is not provided, you must pass: --model-name, --train-file" in result.output
    ...
    def test_train_eval_file_none_override(self, mock_run_training):
        ...
        result = runner.invoke(app, [
            ...
            "--model-name", "test-model",
            "--train-file", "train.jsonl",
            "--eval-file", ""  # Empty string should be treated as None
        ])
...
class TestIntegration:
    ...
    @patch('advsecurenet.llm_finetuning.cli.run_training')
    @patch('advsecurenet.llm_finetuning.cli.load_yaml')
    def test_config_type_preservation_through_pipeline(self, mock_load_yaml, mock_run_training):
        ...
        try:
            ...
            result = runner.invoke(app, [
                ...
                "--config", config_path,
                "--model-name", "new-model",
                "--train-file", "new-train.jsonl",
                "--eval-file", "new-eval.jsonl",
    ...
    def test_flags_only_config_creation(self, mock_run_training):
        ...
        result = runner.invoke(app, [
            "train",
            "--model-name", "flags-model",
            "--train-file", "flags-train.jsonl"
        ])
...
class TestEdgeCases:
    ...
    def test_apply_overrides_filters_out_falsy_values(self):
        ...
        assert result.train.model_name == "test-model"     # Unchanged (empty string filtered)
        assert result.data.eval_file == "eval.jsonl"       # Unchanged (None filtered)
    ...
    def test_train_with_valid_string_values(self, mock_run_training):
        ...
        result = runner.invoke(app, [
            "train",
            "--model-name", "valid-model",  # Use valid model name
            "--train-file", "train.jsonl"
        ])
    ...
    @patch('advsecurenet.llm_finetuning.cli.run_training', side_effect=Exception("Training failed"))
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--cov=advsecurenet.llm_finetuning.cli", "--cov-report=html"])