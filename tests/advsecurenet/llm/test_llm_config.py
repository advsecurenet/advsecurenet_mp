...
class TestDataConfig:
    ...
    def test_validation_error_no_source(self):
        ...
        with pytest.raises(
            ValidationError, match="Provide either data.train_file or data.hub_name"
        ):
            DataConfig()
...
class TestMerge:
    ...
    def test_merge_preserves_unmodified_sections(self):
        ...
        base = Config(
            train=TrainConfig(
                model_name="base-model", learning_rate=2e-4, max_steps=1000
            ),
        ...
        new_train = TrainConfig(
            model_name="new-model", learning_rate=1e-3, max_steps=2000
        )
        override = {"train": new_train}
        result = merge(base, override)
...
if __name__ == "__main__":
    pytest.main(
        [
            __file__,
            "-v",
            "--cov=advsecurenet.llm_finetuning.config",
            "--cov-report=html",
        ]
    )
