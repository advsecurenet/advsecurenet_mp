...
class TestLoadTokenizedDatasets:
    ...
    def test_jsonl_train_only(self, mock_load_dataset):
        ...
        # Verify
        mock_load_dataset.assert_called_once_with("json", data_files={"train": "train.jsonl"})
    ...
    def test_jsonl_train_and_eval(self, mock_load_dataset):
        ...
        # Setup
        cfg = DataConfig(train_file="train.jsonl", eval_file="eval.jsonl", max_seq_len=1024)
        tokenizer = Mock()
        ...
        # Verify
        mock_load_dataset.assert_called_once_with("json", data_files={
            "train": "train.jsonl", 
    ...
    def test_hub_dataset_train_only(self, mock_load_dataset):
        ...
        with patch('advsecurenet.llm_finetuning.data.DatasetDict', return_value=real_dataset_dict):
        ...
        # Verify
        mock_load_dataset.assert_called_with("test-dataset", "main", split="train[:100]")
    ...
    def test_no_data_source_error(self):
        ...
        with pytest.raises(ValueError, match="Provide either JSONL files or a hub dataset"):
...
class TestTextProcessing:
    ...
    def test_prompt_response_fields(self, mock_load_dataset):
        ...
        call_args = tokenizer.call_args[0][0]  # First positional argument (batch["text"])
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--cov=advsecurenet.llm_finetuning.data", "--cov-report=html"])