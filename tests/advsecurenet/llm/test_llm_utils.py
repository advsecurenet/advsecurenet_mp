...
class TestSetSeedAll:
    ...
    @patch('advsecurenet.llm_finetuning.utils.np.random')
    @patch('advsecurenet.llm_finetuning.utils.random')
    def test_deterministic_exceptions_ignored(self, mock_random, mock_np_random, mock_torch):
...
class TestDistInfo:
    ...
    def test_is_rank_zero_true(self):
        ...
        with patch('advsecurenet.llm_finetuning.utils.dist_info', return_value={"rank": 0}):
            assert is_rank_zero() is True
    
    def test_is_rank_zero_false(self):
        """Test is_rank_zero returns False for non-zero rank."""
        with patch('advsecurenet.llm_finetuning.utils.dist_info', return_value={"rank": 1}):
    ...
    def test_rank_zero_only_decorator(self):
        ...
        with patch('advsecurenet.llm_finetuning.utils.is_rank_zero', return_value=False):
            result = test_func(5)
...
class TestLoRATargets:
    ...
    def test_lora_default_targets_common_patterns(self):
        ...
        expected = ["layer.0.attention.q_proj", "layer.0.attention.v_proj", 
                   "layer.1.attention.k_proj", "layer.1.attention.o_proj"]
    ...
    def test_lora_default_targets_mixed_patterns(self):
        ...
        expected = ["model.layers.0.self_attn.query_key_value", 
    ...
    def test_save_json_non_zero_rank(self, mock_is_rank_zero):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            class TestIOUtils:
                ...
                @patch('advsecurenet.llm_finetuning.utils.is_rank_zero')
                def test_save_run_config_pydantic_model(self, mock_is_rank_zero, mock_save_json):
                ...
                @patch('advsecurenet.llm_finetuning.utils.save_json')
                @patch('advsecurenet.llm_finetuning.utils.is_rank_zero')
                def test_save_run_config_dict_like(self, mock_is_rank_zero, mock_save_json):
            ...
            class TestCudaMemory:
                ...
                def test_cuda_mem_with_cuda(self, mock_torch):
                    ...
                    mock_torch.cuda.is_available.return_value = True
                    mock_torch.cuda.memory_allocated.return_value = 1024 * 1024 * 512  # 512 MB
                ...
                def test_cuda_mem_exception_handling(self, mock_torch):
                    ...
                    mock_torch.cuda.is_available.return_value = True
                    mock_torch.cuda.memory_allocated.side_effect = Exception("CUDA error")
            ...
            class TestOptionalDependencies:
                ...
                def test_require_bitsandbytes_if_needed_missing(self, mock_util):
                    ...
                    with pytest.raises(ImportError, match="bitsandbytes is required for QLoRA"):
                        require_bitsandbytes_if_needed(config)
            ...
            class TestEdgeCases:
                ...
                def test_default_dtype_edge_cases(self, mock_torch):
                    ...
                    with patch('advsecurenet.llm_finetuning.utils.bf16_supported', return_value=False):
                        result = default_dtype(prefer_bf16=True)
                        assert result == mock_torch.float32
