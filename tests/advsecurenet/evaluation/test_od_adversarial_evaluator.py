...
def dummy_targets():
    return [
        {"boxes": torch.zeros((1, 4)), "labels": torch.zeros((1,)), "scores": torch.ones((1,))},
        {"boxes": torch.ones((2, 4)), "labels": torch.ones((2,)), "scores": torch.ones((2,))},
    ]

@pytest.fixture
def evaluator():
    # Patch the mean_average_precision evaluator
    evaluator = ObjectDetectorAdversarialEvaluator(evaluators=["mean_average_precision"])
    evaluator.evaluators["mean_average_precision"] = MagicMock()
    return evaluator

def test_update_calls_mean_average_precision(evaluator, dummy_model, dummy_images, dummy_targets):
...
def test_get_results_returns_map(evaluator):
    evaluator.evaluators["mean_average_precision"].get_results.return_value = {"mAP": 0.5}
