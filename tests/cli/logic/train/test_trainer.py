...
def test_cli_trainer_execute_ddp_training_no_gpu_ids(mock_prepare_dataset):
    ...
    with patch('cli.logic.train.trainer.torch.cuda.device_count', return_value=4), \
         patch('cli.logic.train.trainer.set_visible_gpus') as mock_set_gpus, \
         patch('cli.logic.train.trainer.DDPCoordinator') as mock_ddp_coordinator:
...
def test_cli_trainer_execute_ddp_training_empty_gpu_ids(mock_prepare_dataset):
    ...
    with patch('cli.logic.train.trainer.torch.cuda.device_count', return_value=2), \
         patch('cli.logic.train.trainer.set_visible_gpus') as mock_set_gpus, \
         patch('cli.logic.train.trainer.DDPCoordinator') as mock_ddp_coordinator:
...
def test_cli_trainer_prepare_training_environment(mock_prepare_dataset):
    ...
    with patch.object(trainer, '_initialize_model', return_value=mock_model) as mock_init_model, \
         patch.object(trainer, '_prepare_dataloader', return_value=mock_dataloader) as mock_prep_dataloader, \
         patch.object(trainer, '_prepare_train_config', return_value=mock_train_config) as mock_prep_config:
...
def test_cli_trainer_prepare_train_config(mock_prepare_dataset):
    ...
    with patch('cli.logic.train.trainer.TrainingProcessConfig') as mock_training_process_config, \
         patch('cli.logic.train.trainer.ModelConfig') as mock_model_config, \
         patch('cli.logic.train.trainer.TrainConfig') as mock_train_config, \
         patch('cli.logic.train.trainer.asdict') as mock_asdict:
