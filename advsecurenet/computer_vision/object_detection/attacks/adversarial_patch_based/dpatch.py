...
class DPatch(ObjectDetectionAttack):
    ...
    def __init__(self, config: DPatchAttackConfig) -> None:
        ...
        self._patch = torch.zeros(
            (
                tuple(
                    int(x)
                    for x in self.patch_shape.replace("(", "")
                    .replace(")", "")
                    .replace(" ", "")
                    .split(",")
                )
                if isinstance(self.patch_shape, str)
    ...
    def attack(self, dataloader, mask, device, *args, **kwargs) -> torch.tensor:
        ...
        for i_step in trange(self.max_iterations, desc="DPatch iteration"):
            ...
            if self._target_label is not None:
                self._patch = self._patch - self.learning_rate * torch.sign(
                    patch_gradients_sum
                )
            else:
    ...
    def _attack_step(
        ...
        **kwargs,
        ...
        if self._target_label is not None and y is not None:
            warnings.warn(
                f"[DPATCH] Both target_label and y have been provided at the same time. Removing y to avoid conflict."
            )
        ...
        patched_images = self.device_manager.to_device(
            patched_images
        )  # Ensure patched_images is on the correct device
    ...
    def _prepare_patch_targets(self, patched_images, transforms, y):
        ...
        elif self._target_label is None:  # untargetted attack
            predictions = None
            if y is not None:
    ...
    @staticmethod
    def _augment_images_with_patch_transforms_provided(
        transforms, i_image, img_width, img_height, patch_width, patch_height, x_copy
    ):
        i_x_1 = transforms[i_image]["i_x_1"]
        i_x_2 = transforms[i_image]["i_x_2"]
        i_y_1 = transforms[i_image]["i_y_1"]
    ...
    @staticmethod
    def _augment_images_with_patch_random_location_no_mask(
        ...
        if max_h_start < 0 or max_w_start < 0:
            raise ValueError(
                f"Patch (H={patch_height}, W={patch_width}) is larger than image (H={img_height}, W={img_width})."
            )
    ...
    @staticmethod
    def _augment_images_with_patch_random_location_with_mask(
    ...
    @staticmethod
    def _augment_images_with_patch_no_transforms(
        random_location, mask, img_width, img_height, patch_width, patch_height, i_image
    ):
        if random_location:
            if mask is None:
                i_x_1, i_y_1 = (
                    DPatch._augment_images_with_patch_random_location_no_mask(
                        img_width, img_height, patch_width, patch_height
                    )
                )
            else:
                ...
                i_x_1, i_y_1 = (
                    DPatch._augment_images_with_patch_random_location_with_mask(
                        mask, img_width, img_height, patch_width, patch_height, i_image
                    )
                )
        ...
        for i_image in range(x_copy.shape[0]):
            if transforms is None:
                i_x_1, i_x_2, i_y_1, i_y_2 = (
                    DPatch._augment_images_with_patch_no_transforms(
                        random_location,
                        mask,
                        img_width,
                        img_height,
                ...
                i_x_1, i_x_2, i_y_1, i_y_2 = (
                    DPatch._augment_images_with_patch_transforms_provided(
                        transforms,
                        i_image,
                        img_width,
                        img_height,
            ...
            try:
                ...
                if target_slice.shape != patch_copy.shape:
                    raise RuntimeError(
                        f"Shape mismatch before assignment: Slice shape {target_slice.shape}, Patch shape {patch_copy.shape}"
                    )
                ...
                print(
                    f"  Calculated slice shape: ({x_copy.shape[1]}, {i_x_2 - i_x_1}, {i_y_2 - i_y_1})"
                )
                raise e
