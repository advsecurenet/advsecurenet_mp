...
def attack(config, model, attack_type, data_type, device, steps, train_data, batch_size,
           learning_rate, control_init, data_offset, verbose, config_override):
    ...
    if not config_path.exists():
        ...
        if configs_dir.exists():
            available_configs = [f.name for f in configs_dir.glob('*.py') if f.name != '__init__.py']
    ...
    for override in config_override:
        if '=' in override:
            ...
            click.echo(f"⚠️  Invalid config override format: {override} (use key=value)", err=True)
...
def list_configs():
    ...
    config_files = sorted([f for f in configs_dir.glob('*.py') if f.name != '__init__.py'])
...
@click.option('--model', '-m', 
              help='Override model name (default: use from results)')
@click.option('--device', default='auto',
              help='Device to use (auto, cpu, cuda:0)')
@click.option('--temperature', '-t', type=float, default=0.7,
              help='Generation temperature')
@click.option('--num-samples', '-n', type=int, default=3,
              help='Number of response samples')
@click.option('--save-results', '-s', is_flag=True,
              help='Save evaluation results')
@click.option('--verbose', '-v', is_flag=True,
              help='Show all responses')
def evaluate_hf(results_file, model, device, temperature, num_samples, save_results, verbose):
    ...
    cmd = [
        sys.executable, 'eval_scripts/evaluate_hf_attack.py',
