...
def resolve_hf_identifiers(config: model_config.CreateModelConfig) -> Tuple[str, str]:
        ...
        """
        identifier_to_process, identifier_source_field_name = model_config.determine_identifier_and_soruce(config)

        final_model_id = huggingface_general_utils.process_hf_identifier(identifier_to_process)
